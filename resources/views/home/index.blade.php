    ...
    <div class="container mx-auto px-4 pt-16">
        <div class="filmes-populares">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-8">
                @foreach($popularMovies as $filme)

                    <div class="mt-8">
                        <a href="{{route('home.filmes.show',$filme['id'])}}">
                            <img class="hover:opacity-75 transition-ease-in-out duration-150" src="{{'https://image.tmdb.org/t/p/w500/'.$filme['poster_path']}}" alt="Velozes e Furiosos">
                        </a>
                        <div class="mt-2">
                            <a href="#" class="text-lg mt-2 hover:text-purple-300">{{$filme['title']}}</a>
                            ...
                            <div class="text-purple-400 text-sm">
                                @foreach($filme['genre_ids'] as $genero)
                                    {{$genres->get($genero)}}@if(!$loop->last),@endif
                                @endforeach
                            </div>
                        </div>
                    </div>
